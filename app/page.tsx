...
export default async function Home() {
  ...
  const roneyPost = {
    id: 'roney-nemer-2026-09-01', slug: 'roney-nemer-volta-disputa-distrital-experiencia-propostas-brasilia',
    title: { rendered: 'Rôney Nemer: experiência e diálogo pelo Distrito Federal' }, excerpt: { rendered: 'Candidato a deputado distrital pelo PP, número 11111, Rôney Nemer retorna à disputa eleitoral com uma trajetória construída na administração pública, no Legislativo e nas comunidades de Brasília.' },
    date: '2026-09-02T07:45:00-03:00', published_at: '2026-09-02T07:45:00-03:00', created_at: '2026-09-02T07:45:00-03:00', category: 'Política', categorySlug: 'politica', categoryColor: 'bg-red-600', featured_image: 'https://dados.agenciasertao.com/json/v1/eleicoes/2026/fotos/70002538503.jpg', href: '/noticia/roney-nemer-volta-disputa-distrital-experiencia-propostas-brasilia',
  };
  ...
  const isPolitica = (p: any) => p?.categorySlug === 'politica' || /politica|lula|celina|michelle bolsonaro|julio cesar|flavio bolsonaro|leila|roney nemer|augusto cury|bolsonaro|caiado|zema|congresso|presidencia|eleicoes 2026|planalto|buriti/.test(norm(p));
  const politicaPosts = posts.filter(isPolitica); const maceioPosts = posts.filter(isMaceio); const fotoRuim = (p: any) => !p?.featured_image || /\.(gif)$/i.test(String(p.featured_image));
  const obrigatorios = ['julio cesar','roney nemer','celina','leila','michelle bolsonaro','lula','flavio bolsonaro'];
  const base = (politicaPosts.length >= 3 ? politicaPosts : posts).filter((p: any) => !fotoRuim(p)); const destaques: any[] = [];
  obrigatorios.forEach((termo) => { const achado = base.find((p: any) => norm(p).includes(termo) && !destaques.includes(p)); if (achado) destaques.push(achado); });
  const recentes = [...base].sort((a: any,b: any) => new Date(b?.published_at || b?.created_at || b?.date || 0).getTime() - new Date(a?.published_at || a?.created_at || a?.date || 0).getTime()); recentes.forEach((p:any)=>{if(!destaques.includes(p))destaques.push(p)}); const heroPosts=destaques.slice(0,8);
